...
const extraerYGuardarBicicletas = async () => {
    try {
      ...
      for (const bicicletas of datos) {
        try {
          // Columnas: 'nombre', 'edad', 'raza' (ajusta si los headers son diferentes)
          await addDoc(collection(db, "bicicletas"), {
            marca: bicicletas.marca || "",
            modelo: bicicletas.modelo || 0,
            tipo: bicicletas.tipo || "",
            precio: parseInt(bicicletas.precio) || 0,
            color: bicicletas.color || "",
          });
          guardados++;
        } catch (err) {
          console.error("Error guardando bicicleta:", bicicletas, err);
          errores++;
        }
      }
  ...
  return (
    <View style={styles.container}>
      ...
      <Button title="Importar Bicicletas desde Excel" onPress={extraerYGuardarBicicletas} />
      <FormularioProductos
